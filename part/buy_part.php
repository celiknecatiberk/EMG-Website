<?php
	...
	if (isset($_POST['name'])) {
		if (!empty($_POST['name']) && !empty($_POST['email']) && !empty($_POST['mobile']) && !empty($_POST['city']) && !empty($_POST['pincode']) && !empty($_POST['address'])) {
            ...
            {
                $database->Query("UPDATE product SET stock=stock-1 WHERE `id`='$term'");
            }
			$insert = $database->Insert("buy","'','$user_id','$name','$email','$mobile','$city','$pincode','$address','$time','','Processing','1','$product_id_stack'");

			if ($insert) {
				$buy_id = mysqli_insert_id($mysqli);
				$update = $database->Query("UPDATE `cart` SET `active`='n',buy_id='$buy_id' WHERE `user_id`='$user_id'");
				if ($update) {
					$result['type'] = "success";
				}else{
					$result['type'] = "error";
					$result['message'] = "The basket could not be updated please try again";

					$delete = $database->Delete("buy","id='$buy_id'");
				}
			}else{
				$result['type'] = "error";
